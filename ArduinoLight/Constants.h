...
#define NUM_LEDS 16           //количество светодиодов на ленте
...
#define  DEMOTIMELIMIT ( 1 * 5UL * 1000UL)   //  минуты/секунды/милисекунды время задержка между эффектами
uint32_t DemTimer = 0UL;                      // тут будет храниться время следующего переключения эффекта
bool isDemo = true;                          // вкл/выкл демо при включении
//-----------Переменные-----------------
byte hue;
uint16_t position;
boolean direction;
